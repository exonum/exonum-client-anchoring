...
export class Anchoring {
  ...
  parseOpReturn (opReturn) {
    ...
    return {
      version: byteArrayToInt(exonum.hexadecimalToUint8Array(anchor.slice(0, 2))),
      payloadType: byteArrayToInt(exonum.hexadecimalToUint8Array(anchor.slice(2, 4))),
      blockHeight: byteArrayToInt(exonum.hexadecimalToUint8Array(anchor.slice(4, 20))),
   ...
   */
  getActualAddress () {
    return axios.get(`${this.anchoringPath}/address/actual`).then(({ data }) => data)
  }
  ...
  getBlock (blockHeight) {
    return axios.get(`${this.explorerPath}/blocks/${blockHeight}`).then(({ data }) => data)
  }
